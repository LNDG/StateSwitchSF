...
% - duration always needs to be positive (vs. SPM convention of 0 for stick)
...
% v4: concatenated across sessions (see https://en.wikibooks.org/wiki/SPM/Concatenation)
% v5 (current): Remove -1 from cond onsets. AP also put in his specific files and data directories
...
for indID = 1:numel(IDs)
    ...
    for indSession = 1:numOfRuns
        % get regressor data from single run
        pn.regressors = '/Volumes/LNDG/Projects/StateSwitch/dynamic/data/mri/task/analyses/A_extractDesignTiming/B_data/A_regressors/';
        tmp = load([pn.regressors, IDs{indID}, '_Run',num2str(indSession),'_regressors.mat']);
        tmpRegressors{indSession} = tmp.Regressors; clear tmp;
    end; clear indSession;
    % concatenate run regressors
    Regressors = cat(1,tmpRegressors{:}); clear tmpRegressors;
    ...
    for indSession = 1:numOfRuns
        ...
        N(indSession) = size(basefileInfo,1);
        for indVol = 1:N(indSession)
            allFiles{indSession}{indVol,1} = [basefile, ',', num2str(indVol)];
        end; clear indVol
    end; clear indSession
    allFiles = cat(1, allFiles{:});
    indSession = 1;
    matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).scans = allFiles; clear allFiles;
    indCond = 1;
    for indDim = 1:4
        for indAtt = 1:4
        matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).name = ['StimOnset_dim',num2str(indDim), '_Att',num2str(indAtt)];
        %matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).onset = find(Regressors(:,3) == 1 & Regressors(:,4) == indDim & Regressors(:,6+indAtt) == 1)-1; % IMPORTANT: SPM starts counting at 0
        matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).onset = find(Regressors(:,3) == 1 & Regressors(:,4) == indDim & Regressors(:,6+indAtt) == 1); % IMPORTANT: AP remove -1
    ...
    %matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).onset = find(Regressors(:,2) == 1)-1; % IMPORTANT: SPM starts counting at 0
matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).onset = find(Regressors(:,2) == 1)-1; % IMPORTANT: AP removes -1
    onsets = matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).onset;
    ...
    %matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).onset = find(Regressors(:,11) == 1)-1; % IMPORTANT: SPM starts counting at 0
matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).onset = find(Regressors(:,11) == 1)-1; % IMPORTANT: AP removes -1
    onsets = matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).onset;
    matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).duration = repmat(0, numel(onsets), 1); clear onsets; % duration of 1 (VarToolbox) vs 0 (SPM convention)
    matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).tmod = 0;
    matlabbatch{1}.spm.stats.fmri_spec.sess(indSession).cond(indCond).pmod = struct('name', {}, 'param', {}, 'poly', {});
    ...
    save([pn.root, 'B_data/D_batchFiles1stLevelGLM-fix/',IDs{indID},'_SPM1stBatchGLM.mat'], 'matlabbatch', 'N');
